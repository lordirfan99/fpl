...
export default async function MyTeamPage() {
  ...
  const recHeld = !rec || rec.freshness.stale || !rec.freshness.accountStateVerified || rec.packetStatus === "safe_hold" || rec.packetStatus === "needs_refresh";
  ...
  return <div className="page-stack">
    ...
    <ChipNotice targetGameweek={targetGameweek} liveGameweek={live?.gameweek} liveActiveChip={live && live.gameweek === season.liveGw && season.liveGw > season.finalizedGw ? live.active_chip : null} />
    <section className="metric-grid"><MetricCard label={`GW${displayGameweek} points`} value={liveTeam && livePoints != null ? `${livePoints}` : `${manager.gw_points}`} detail={liveTeam ? "Official live · provisional" : "Completed snapshot"} tone={liveTeam ? "warning" : undefined} /><MetricCard label="Overall points" value={`${liveTeam && liveTotal != null ? liveTotal : manager.total_points}`} detail={`Overall rank #${(liveTeam && liveOverallRank ? liveOverallRank : manager.overall_rank).toLocaleString()}`} /><MetricCard label="League rank" value={`#${liveTeam && liveLeagueRank ? liveLeagueRank.toLocaleString() : manager.league_rank.toLocaleString()}`} detail={liveTeam && live?.league ? `${live.league.name} · ${live.league.rank_count.toLocaleString()} teams` : "Latest completed snapshot"} /><MetricCard label="Next deadline" value={`GW${targetGameweek}`} detail={formatMYT(deadline) ?? "Not published"} tone={historical ? "warning" : undefined} /></section>
    <div className="content-grid team-layout"><section className="surface pitch-surface"><div className="section-heading"><div><span>{liveTeam ? "OFFICIAL LIVE STARTING XI" : "CAPTURED STARTING XI"}</span><h2>{historical ? `GW${data.gameweek} review` : liveTeam ? `GW${displayGameweek} team` : "Pitch view"}</h2></div><span className="section-chip">11 + 4</span></div><Pitch squad={squad} bootstrap={data.bootstrap} metaForPick={liveTeam ? playerMeta : undefined} /></section><aside className="insight-rail"><section className="surface insight-card primary"><span>NEXT DEADLINE ACTION</span><h2>{move ? `${move.outgoing.name} → ${move.incoming.name}` : recHeld ? "Check your verified plan" : "Set lineup, hold transfer"}</h2><p>{recHeld ? "Public gameweek picks may omit transfers since the deadline. Check your current account and latest Telegram plan." : rec?.competitive.phaseReason}</p><a href="/assistant">Open decision board</a></section><section className="surface insight-card"><span>PLANNING TARGET</span><strong>GW{targetGameweek}</strong><p>Use this target across Assistant, Planner and Players. Every change is applied manually in FPL.</p></section><section className="surface insight-card"><span>AVAILABILITY · PUBLIC GW{displayGameweek} SQUAD</span><strong>{flagged.length ? "Check flags" : "No flags reported"}</strong><p>{flagged.join(", ") || "Verify late team news and your current squad before the next deadline."}</p></section></aside></div>
  </div>;
}
