...
export default async function PlannerPage({ searchParams }: { searchParams: Promise<{ gw?: string }> }) {
  ...
  return <div className="page-stack"><PageHeader eyebrow={`FIXTURE RESEARCH · START GW${data.fromGameweek}`} title="Explore the next five gameweeks" description={`Official FPL fixtures and difficulty for GW${data.fromGameweek}–GW${data.toGameweek}. This page is research, not a verified personal recommendation.`} />
    <section className="surface planner-note"><span>{targetGameweek ? "HISTORICAL / FUTURE RESEARCH" : "PERSONAL PLAN UNAVAILABLE"}</span><h2>No transfer or hold recommendation is shown</h2><p>{targetGameweek ? `You selected GW${targetGameweek}. Use these recorded fixtures for research only.` : "Unlock the private dashboard and wait for a current verified VM packet to use the decision plan."}</p><p><Link href={privatePlan?.status === "signed_out" ? "/sign-in" : "/this-week"}>{privatePlan?.status === "signed_out" ? "Unlock private plan" : "Check plan status"}</Link></p></section>
