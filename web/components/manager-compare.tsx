...
export function ManagerCompare({ directory, left, right, bootstrap, gameweek, leagueId }: { directory: ManagerSummary[]; left: Manager; right: Manager; bootstrap: Bootstrap; gameweek: number; leagueId: number }) {
  ...
  const select = (side: "left" | "right", id: number) => {
    ...
    // Keep both squads on screen while the new pair streams in.
    router.prefetch(`/compare?${params}`);
    startTransition(() => router.push(`/compare?${params}`, { scroll: false }));
  };
  ...
  return <div className={pending ? "is-pending" : undefined} aria-busy={pending || undefined}><div className="compare-selectors"><ManagerPicker label="Manager A" managers={directory} selectedId={left.entry_id} onSelect={(id) => select("left", id)} /><span className="compare-vs">VS</span><ManagerPicker label="Manager B" managers={directory} selectedId={right.entry_id} onSelect={(id) => select("right", id)} /></div>
